...
"""
MinMaxScaler(data::AbstractDataFrame; features=:, outrange::NTuple{2, <:Real}=(0., 1.)) = MinMaxScaler{outrange}(data[!, features] |> Matrix; features=features)
MinMaxScaler(data::AbstractMatrix{<:Real}; features=:, outrange::NTuple{2, <:Real}=(0., 1.)) = MinMaxScaler{outrange}(data[:, features]; features=features)
