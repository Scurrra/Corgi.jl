...
"""
StandardScaler(data::AbstractDataFrame; features=:, with_μ::Bool=true, with_σ::Bool=true, outrange::NTuple{2, <:Real}=(0., 1.)) = StandardScaler{outrange}(data[!, features] |> Matrix; features=features, with_μ=with_μ, with_σ=with_σ)
StandardScaler(data::AbstractMatrix{<:Real}; features=:, with_μ::Bool=true, with_σ::Bool=true, outrange::NTuple{2, <:Real}=(0., 1.)) = StandardScaler{outrange}(data[:, features]; features=features, with_μ=with_μ, with_σ=with_σ)
