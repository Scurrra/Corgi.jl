...
"""
MaxAbsScaler(data::AbstractDataFrame; features=:, outrange::NTuple{2,<:Real}=(-1.0, 1.0)) = MaxAbsScaler{outrange}(data[!, features] |> Matrix; features=features)
MaxAbsScaler(data::AbstractMatrix{<:Real}; features=:, outrange::NTuple{2,<:Real}=(-1.0, 1.0)) = MaxAbsScaler{outrange}(data[:, features]; features=features)
