    ...
    if !opt.early_stopping
        ...
        for epoch in 1:opt.epochs
            ...
            for indx in Utils.split(indexes(), opt.batchsize)
                u = opt.γ * v .+ lr * (opt.∇J(model.ω, data[indx], target) .+ opt.regularizer(model.ω)[2] / length(indx))
                model.ω .-= u
                v = u
            end
        ...
        for epoch in 1:opt.epochs
            lr = opt.lrscheduler(epoch)
            for indx in Utils.split(indexes(), opt.batchsize)
                u = opt.γ * v .+ lr * (opt.∇J(ω, data[indx], target) .+ opt.regularizer(ω)[2] / length(indx))
                ω .-= u
                v = u
            end
