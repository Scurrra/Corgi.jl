...
"""
    Adam <: AbstractOptimizer

Adaptive Moment Estimation, `batchsize` equals 1 by default. 
"""
    ...
    if !opt.early_stopping
        ...
        for epoch in 1:opt.epochs
            ...
            for indx in Utils.split(indexes(), opt.batchsize)
                ...
                model.ω .-= lr / sqrt((v / (1 - opt.β₂^epoch)) + ϵ) / (1 - opt.β₁^epoch) * m
            end
        ...
        for epoch in 1:opt.epochs
            ...
            for indx in Utils.split(indexes(), opt.batchsize)
                ...
                m = opt.β₁ * m .+ (1 - opt.β₂) * g
                v = opt.β₂ * v + (1 - opt.β₂) * sum(x->x^2, g)
                ω .-= lr / sqrt((v / (1 - opt.β₂^epoch)) + ϵ) / (1 - opt.β₁^epoch) * m
            end
